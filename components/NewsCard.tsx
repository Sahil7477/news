    ...
    <article className={`bg-white rounded-lg overflow-hidden hover:shadow-lg transition-all duration-300 border border-gray-100 ${className}`}>
      {image && (
        <div className={`relative ${isMainStory ? "aspect-video" : "aspect-[4/3]"} overflow-hidden group`}>
          <Image src={image} alt={title} fill className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />

          <div className="absolute top-3 left-3 flex gap-2">
            {isBreaking && (
              <Badge className="bg-red-600 text-white font-bengali animate-pulse">ব্রেকিং</Badge>
            )}
            {isLive && (
            ...
            <Badge variant="secondary" className="font-bengali bg-black/70 text-white">{category}</Badge>
          </div>
        ...
        <div className="flex items-center justify-between mb-2">
          {!image && (
            <div className="flex gap-2">
              {isBreaking && (
                <Badge className="bg-red-600 text-white font-bengali text-xs">ব্রেকিং</Badge>
              )}
              {isLive && (
          ...
          {timestamp && <div className="news-meta text-meta-text">{timestamp}</div>}
        </div>

        <h3 className={`news-headline mb-3 hover:text-news-red cursor-pointer transition-colors line-clamp-3 ${isMainStory ? "text-xl md:text-2xl" : "text-sm md:text-base"}`}>
          {title}
        </h3>

        {description && <p className="text-subheadline text-sm mb-3 line-clamp-2">{description}</p>}
          ...
          <div className="flex items-center space-x-2">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent className="bg-white border border-gray-200 shadow-lg z-50">
                <DropdownMenuItem onClick={() => handleShare("facebook")} className="font-bengali">Facebook এ শেয়ার করুন</DropdownMenuItem>
                <DropdownMenuItem onClick={() => handleShare("twitter")} className="font-bengali">Twitter এ শেয়ার করুন</DropdownMenuItem>
                <DropdownMenuItem onClick={() => handleShare("whatsapp")} className="font-bengali">WhatsApp এ শেয়ার করুন</DropdownMenuItem>
                <DropdownMenuItem onClick={() => handleShare("copy")} className="font-bengali">লিংক কপি করুন</DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
          </div>
        </div>
